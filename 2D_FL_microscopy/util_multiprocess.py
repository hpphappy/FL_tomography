...
def self_absorption_ratio_single_theta_voxel(theta, P, src_path, sample_size, sample_size_l, n_theta, theta_ls, element_ls, att_cs_lib, n_det_energy_bins, n_det, det_pos_ls, j):
    ...
    ## calculate the attenuation averaged all paths for all energy chennels, dimension = n_energy_bins
...
def self_absorption_ratio(src_path, sample_size, sample_size_l, n_theta, theta_ls, element_ls, att_cs_lib, n_det_energy_bins, n_det, det_pos_ls):
    ...
    for i, theta in enumerate(tqdm(theta_ls.tolist(), leave=True)):
        pfunc = partial(self_absorption_ratio_single_theta_voxel, theta, P, src_path, sample_size, sample_size_l, n_theta, theta_ls, element_ls, att_cs_lib, n_det_energy_bins, n_det, det_pos_ls)
        with Pool(5) as p:            
            SA_i  = np.array(p.map(pfunc, np.arange(sample_size * sample_size)))                 
        SA[i,:,:] += SA_i             
    return SA
